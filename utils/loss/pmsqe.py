...
class SingleSrcPMSQE(nn.Module):
    ...
    def forward(self, est_targets, targets, pad_mask=None):
        ...
        if pad_mask is not None:
            ...
            pad_mask = torch.ones(
                est_targets.shape[0], est_targets.shape[1], 1
            ).to(est_targets.device)
        # SLL equalization
        ...
        pmsqe = 4.5 - torch.sum(pmsqe_frame, dim=dims) / pad_mask.sum(dims)
        return pmsqe
    ...
    def populate_constants(self, sample_rate):
        ...
        self.mask_sll = nn.Parameter(tensor(mask_sll))
