...
class TFGridNet(nn.Module):
    ...
    def __init__(
        ...
        self.V_TCN = nn.ModuleList([])
        for _ in range(5):
            self.V_TCN.append(VisualConv1D(V=65, H=130))

        self.deconv = nn.ConvTranspose2d(emb_dim*2, n_srcs * 2, ks, padding=padding)
        
        # self.ln = nn.Linear(256, n_freqs)
        # self.v_conv2 = nn.Conv1d(256,65,1)
        
        # self.ln1 = nn.ModuleList([])
        # for _ in range(n_layers):
        #     self.ln1.append(nn.Linear(256, n_freqs))
            
        # self.ln2 = nn.ModuleList([])
        # for _ in range(n_layers):
        #     self.ln2.append(nn.Linear(256, n_freqs))
        
        # self.norm1 = nn.ModuleList([])
        # for _ in range(n_layers):
        #     self.norm1.append(nn.LayerNorm(n_freqs))
            
        # self.norm2 = nn.ModuleList([])
        # for _ in range(n_layers):
        #     self.norm2.append(nn.LayerNorm(n_freqs))
        
        # self.norm3 = nn.ModuleList([])
        # for _ in range(n_layers):
        #     self.norm3.append(nn.LayerNorm(n_freqs))
    ...
    def forward(
        ...
        # lip_emb = self.v_conv(lip_emb.transpose(1, 2)) # [B, E, T']
        # for ii in range(5):
        #     lip_emb = self.V_TCN[ii](lip_emb)
        # lip_emb = self.v_conv2(lip_emb) # [B, F, T']
        
        # # lip_emb = self.lip_conv(lip_emb)
        # lip_emb = lip_emb.unsqueeze(1).repeat(1,2,1,1)
        # lip_emb = self.conv(lip_emb)
        # lip_emb = lip_emb.transpose(2, 3) # [B, C, T', F]
        # # print('lip', lip_emb.shape)
        
        # lip_emb = self.att(batch, lip_emb)
        # batch = torch.cat([batch, lip_emb], dim=1)
        
        lip_emb = F.interpolate(lip_emb.transpose(1, 2), size=(n_frames), mode='linear') # [B, E, T]
        lip_emb = self.v_conv(lip_emb.squeeze(1)) # [B, F, T]
        for ii in range(5):
            lip_emb = self.V_TCN[ii](lip_emb) # [B, F, T]
        lip_emb = lip_emb.transpose(1, 2).contiguous().unsqueeze(1) # [B, 1, T, F]
        batch = self.conv(batch) # [B, C, T, F]
        lip_emb = self.conv1(lip_emb) # [B, C, T, F]
        # lip_emb = self.ln(lip_emb)
        
        embd_av = self.att_av(batch, lip_emb)
        # embd_va = self.att_va(lip_emb, batch)
        # embd = embd_av + embd_va
        batch  = torch.cat([batch, embd_av], dim=1)
...
class GridNetBlock(nn.Module):
    ...
    def __init__(
        ...
        in_channels = emb_dim * 2 * emb_ks
        

        self.intra_norm = LayerNormalization4D(emb_dim*2, eps=eps)
        self.intra_rnn = nn.LSTM(
            in_channels, hidden_channels, 1, batch_first=True, bidirectional=True
        )
        self.intra_linear = nn.ConvTranspose1d(
            hidden_channels * 2, emb_dim*2, emb_ks, stride=emb_hs
        )

        self.inter_norm = LayerNormalization4D(emb_dim*2, eps=eps)
        ...
        for ii in range(n_head):
            self.add_module(
                "attn_conv_Q_%d" % ii,
                nn.Sequential(
                    nn.Conv2d(emb_dim*2, E, 1), # [B, 1, T, Q]
                    get_layer(activation)(),
            ...
            self.add_module(
                "attn_conv_K_%d" % ii,
                nn.Sequential(
                    nn.Conv2d(emb_dim*2, E, 1), # [B, C, T, Q]
                    get_layer(activation)(),
                    LayerNormalization4DCF((E, n_freqs), eps=eps),
                ),
            )
            self.add_module(
                "attn_conv_V_%d" % ii,
                nn.Sequential(
                    nn.Conv2d(emb_dim*2, emb_dim*2 // n_head, 1),
                    get_layer(activation)(),
                    LayerNormalization4DCF((emb_dim*2 // n_head, n_freqs), eps=eps),
                ),
            )
        self.add_module(
            "attn_concat_proj",
            nn.Sequential(
                ...
                get_layer(activation)(),
                LayerNormalization4DCF((emb_dim*2, n_freqs), eps=eps),
            ),
        )
    ...
    def forward(self, x, lip_emb):
        ...
        Q = torch.cat(all_Q, dim=0)  # [B', 1, T, C*Q]
