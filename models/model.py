...
class TFGridNet(nn.Module):
    ...
    def __init__(
        ...
        self.v_conv = nn.Conv1d(512,256,1)
        self.V_TCN = nn.ModuleList([])
        for _ in range(5):
            self.V_TCN.append(VisualConv1D(V=256, H=512))

        self.deconv = nn.ConvTranspose2d(emb_dim, n_srcs * 2, ks, padding=padding)
        
        self.ln = nn.Linear(256+n_freqs, n_freqs)
    ...
    def forward(
        ...
        lip_emb = F.interpolate(lip_emb.unsqueeze(1), size=(batch.shape[-2], lip_emb.shape[-1]), mode='nearest') # [B, 1, T, E]
        lip_emb = lip_emb.squeeze(1).permute(0,2,1).contiguous()
        lip_emb = self.v_conv(lip_emb)
        for ii in range(5):
            lip_emb = self.V_TCN[ii](lip_emb)
        lip_emb = lip_emb.permute(0,2,1).contiguous().unsqueeze(1)
...
class GridNetBlock(nn.Module):
    ...
    def forward(self, x, lip_emb):
        ...
        all_Q, all_K, all_V = [], [], []
        for ii in range(self.n_head):
            all_Q.append(self["attn_conv_Q_%d" % ii](batch))  # [B, 1, T, C*Q]
